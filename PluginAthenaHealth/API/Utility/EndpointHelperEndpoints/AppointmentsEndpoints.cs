...
{
    public static class AppointmentsFunctions
    {
        public static async Task<Schema> GetStaticSchemaAsync(IApiClient apiClient, Schema schema,
            Schema.Types.DataFlowDirection dataFlowDirection)

        {
            ...
            schema.DataFlowDirection = dataFlowDirection;
                ...
                do
                {
                    ...
                    if (bookedApptResponse.Appointments != null)
                    {
                        foreach (var bookedAppointment in bookedApptResponse.Appointments)
                        {
                            ...
                            // Vitals Query
                            var vitalsPath =
                                $"{settings.GetBaseUrl().TrimEnd('/')}/{settings.PracticeId}/chart/{bookedAppointment.PatientId.ToString()}/vitals?departmentid={thisDepartmentId}";

                            var vitalsResult = await apiClient.GetAsync(vitalsPath);

                            if (!vitalsResult.IsSuccessStatusCode)
                            {
                                throw new Exception(vitalsResult.Content.ReadAsStringAsync().ToString());
                            }

                            var vitalsResponse =
                                JsonConvert.DeserializeObject<VitalsResponse>(
                                    await vitalsResult.Content.ReadAsStringAsync());

                            var mostRecentHeight = vitalsResponse.Vitals
                                .Find(x => x.Key == Constants.VitalsHeightKey)
                                ?.Readings.SelectMany(x => x)
                                .Where(x => x.Key == Constants.VitalsHeightReadingKey)
                                .OrderByDescending(x => x.Date)
                                .First() ?? null;

                            var mostRecentWeight = vitalsResponse.Vitals
                                .Find(x => x.Key == Constants.VitalsWeightKey)
                                ?.Readings.SelectMany(x => x)
                                .Where(x => x.Key == Constants.VitalsWeightReadingKey)
                                .OrderByDescending(x => x.Date)
                                .First() ?? null;

                            recordMap["patient.height"] = mostRecentHeight?.Value ?? "";
                            recordMap["patient.height.unit"] = mostRecentHeight?.Unit ?? "";
                            recordMap["patient.height.date"] = mostRecentHeight?.Date.ToString("MM/dd/yyyy") ?? "";
                            recordMap["patient.weight"] = mostRecentWeight?.Value ?? "";
                            recordMap["patient.weight.unit"] = mostRecentWeight?.Unit ?? "";
                            recordMap["patient.weight.date"] = mostRecentWeight?.Date.ToString("MM/dd/yyyy") ?? "";
    ...
    public class AppointmentsEndpointHelper
    {
        private class BookedAppointmentsEndpoint_Historical : Endpoint
        {
            ...
            {
                return await AppointmentsFunctions.GetStaticSchemaAsync(apiClient, schema, GetDataFlowDirection());
            }
                ...
                bool isDiscoverRead = false)
            {
                ...
                var records = AppointmentsFunctions.ReadRecordsAsync(apiClient, schema, startDate, endDate);
